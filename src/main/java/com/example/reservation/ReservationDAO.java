...
public class ReservationDAO {
	private static final List<Reservation> reservations = new CopyOnWriteArrayList<>(); private static final AtomicInteger idCounter = new AtomicInteger(0);
	...
	public boolean updateReservation(int id, String name, LocalDateTime reservationTime) { 
		...
		for (int i = 0; i < reservations.size(); i++) {
			if (reservations.get(i).getId() == id) {
				reservations.set(i, new Reservation(id, name, reservationTime)); saveReservations();
				return true;
			} 
		}
		return false;
	...
	public List<Reservation> searchAndSortReservations(String searchTerm, String sortBy, String sortOrder) {
		...
		.filter(r -> searchTerm == null || searchTerm.trim().isEmpty() ||
			...
			r.getReservationTime().format(FORMATTER).contains(searchTerm)) .collect(Collectors.toList());
